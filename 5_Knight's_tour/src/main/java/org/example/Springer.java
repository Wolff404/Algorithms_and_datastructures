...
public class Springer {
    ...
    public static boolean springertur(int i, int j) {
        ...
        //Definer mulige trekk
        int xAkse[]= {2, 2, -2, -2, 1, 1, -1, -1};
        int yAkse[]= {1, -1, 1, -1, 2, -2, 2, -2};

        //For løkke som itererer igjennom de mulige trekk
        for (int k = 0; k < 8; k++){
            int flyttX = i + xAkse[k];
            int flyttY = j + yAkse[k];

        //En if test i løkken for å sjekke om flyttet er lovlig
            if (lovlig(flyttX,flyttY) && springertur(flyttX,flyttY) ){
